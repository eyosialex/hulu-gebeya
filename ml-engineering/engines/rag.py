...
# Synonym table: maps ML intent category labels → possible DB category values
_CATEGORY_SYNONYMS = {
    "health":      ["health", "clinic", "hospital", "pharmacy", "medical", "wellness", "fitness", "services"],
    "food":        ["food", "cafe", "restaurant", "bar", "juice", "hotel"],
    "game":        ["game", "gaming", "arcade", "entertainment"],
    "culture":     ["culture", "church", "mosque", "religion", "art", "museum"],
    "transport":   ["transport", "bus", "taxi", "station"],
    "study":       ["study", "school", "university", "education", "library", "college", "training"],
    "services":    ["services", "bank", "atm", "shop", "market", "repair", "printing"],
    "hidden_gems": ["hidden_gems", "park", "garden", "attraction", "hidden", "gem"],
}

def local_search(query: str, intent: dict, lat=None, lng=None, locations_list=None) -> list:
    """Search the local database with intent-aware filtering and fuzzy category matching."""
    query_clean = clean_query(query)
    cat_label   = intent.get("category")
    keywords    = intent.get("keywords", [])

    # Build set of valid DB categories from the intent label (fuzzy match)
    valid_cats = set()
    if cat_label:
        valid_cats.update(_CATEGORY_SYNONYMS.get(cat_label, [cat_label]))

    results = []
    source = locations_list if locations_list is not None else []
    for loc in source:
        loc_cat = loc.get("category", "").lower()
        loc_name = loc.get("name", "").lower()

        # Match if DB category is in the synonym set
        match_cat  = bool(valid_cats) and loc_cat in valid_cats
        # Match if any search keyword appears in the location name
        match_name = any(kw in loc_name for kw in keywords)
        # Match if any keyword appears directly in the category value (e.g., "clinic" in query)
        match_cat2 = any(kw in loc_cat for kw in keywords)
        # Match if the query text contains the category label itself
        match_cat3 = loc_cat in query_clean

        if match_cat or match_name or match_cat2 or match_cat3:
            results.append({**loc, "source": "database"})

    for r in results:
        r["distance"] = calculate_distance(lat, lng, r.get("lat"), r.get("lng"))
...
def retrieve(query: str, intent: dict, lat=None, lng=None, fast_mode=False, locations_list=None) -> dict:
    """Combine all sources: Database (verified), OSM, and Overpass."""
    
    # 1. Database Search (Prioritized)
    local = local_search(query, intent, lat=lat, lng=lng, locations_list=locations_list)
    
    # Run Trust Engine on local results
    for r in local:
    ...
    # 2. External Search (OSM & Overpass)
    # We include these if local results are insufficient OR if fast_mode is false
    osm = []
    poi = []
    if not fast_mode or len(local) < 3:
